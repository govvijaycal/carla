...
class FrenetTrajectoryHandler(object):
	def __init__(self, way_s, way_xy, way_yaw, s_resolution=0.5, debug=False):
	...
	def _generate_frenet_reference_trajectory(self, way_s, way_xy, way_yaw, s_resolution=0.5, debug=False):
		...
		curv_frenet_filt = filtfilt(np.ones((3,))/3, 1, curv_frenet_raw)   # curvature filtering
		curv_frenet_filt = np.insert(curv_frenet_filt, len(curv_frenet_filt), 0.0)
		...
		if debug:
			...
			plt.figure()
			plt.plot(s_frenet, curv_frenet_filt, 'b', label='filt')
		...
		return s_frenet, x_frenet, y_frenet, yaw_frenet, curv_frenet_filt

	def convert_global_to_frenet_frame(self, x_query, y_query, yaw_query):
		...
		yaw_waypoint = self.trajectory[closest_index, 3]
		...
		psi_error = psi_query - psi_waypoint
